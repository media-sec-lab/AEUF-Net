...
def _sample_rois(all_rois, all_scores, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    # print(11,all_rois[:, 1:5],gt_boxes[:, :4])
    ...
    if fg_inds.size > 0 and bg_inds.size > 0:
        ...
        bg_inds = np.where((max_overlaps < cfg.FLAGS.roi_bg_threshold_high) & (max_overlaps >= 0))[0]
        to_replace = bg_inds.size < rois_per_image
        # print(6,max_overlaps)
        # print(1,bg_inds)
        # print(2,rois_per_image,int(rois_per_image))
        # print(3,to_replace)
        bg_inds = npr.choice(bg_inds, size=int(rois_per_image), replace=to_replace)
        fg_rois_per_image = 0
    ...
    bbox_targets, bbox_inside_weights = \
        _get_bbox_regression_labels(bbox_target_data, num_classes)  #box的回归偏移量bbox_targets
