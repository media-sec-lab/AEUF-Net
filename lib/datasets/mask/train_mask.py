...
# from lib.nets.res_diff_2 import resnetv3
# from lib.nets.res_align_rpnsam_b1_3 import resnetv3
from lib.nets.b1_mask import resnetv3
...
class Train:
    def __init__(self):
        ...
        self.tfmodel = '/data/cxm/code/Image_manipulation_detection-master/default/coco_train_filter_single/res_align_rpnsam_b1_3_xin7/resnet101_faster_rcnn_iter_145000.ckpt'
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            if iter % (cfg.FLAGS.display) == 0:
                if cfg.FLAGS.USE_MASK is True:
                    ...
                    print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                          '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> lr: %f' % \
                          (iter, cfg.FLAGS.max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, lr.eval()))
                    print('speed: {:.3f}s / iter'.format(timer.average_time))
                    print('remaining time: {:.3f}h'.format(((cfg.FLAGS.max_iters - iter) * timer.average_time) / 3600))
                    # print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                    #       '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n ' % \
