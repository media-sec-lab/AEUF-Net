...
class coverage(imdb):
  def __init__(self, image_set, year, dist_path=None):
    ...
    self._year = year
    self._image_set = image_set.split('coverage_')[1]
  ...
  def mask_path_from_index(self, index):
    """
    Construct an image path from the image's "index" identifier.   ����ͼ��ľ���·��/.../.../...png
    """
    ...
    for ext in self._image_ext:
      #image_path = os.path.join('/home-3/pengzhou@umd.edu/work/xintong/medifor/portrait/test_data',
                              #index + ext)
      mask_path = os.path.join(r'E:\Server_backup\data\coverage\\mask',
                              mask_name + ext)
  ...
  def _get_voc_results_file_template(self):
    ...
    filename = 'coverage_' + self._image_set + '_{:s}.txt'
    path = os.path.join(
      '.',
      filename)
    return path
  ...
  def _get_voc_noise_results_file_template(self):
    # VOCdevkit/results/VOC2007/Main/<comp_id>_det_test_aeroplane.txt
    filename = 'coverage_' + self._image_set + '_{:s}_noise.txt'
    path = os.path.join(
      '.',
      filename)
    return path
