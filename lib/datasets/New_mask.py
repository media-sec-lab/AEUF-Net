...
class New_3(imdb):
  def __init__(self, image_set, year, dist_path=None):
    ...
    self._year = year
    self._image_set = image_set.split('New_')[1]#现在的3type
    ...
    #                  'removal',
    #                  'splice')
    self._classes = ('authentic',  'splicing','Removal','copymove')
  ...
  def mask_path_from_index(self, index):
    ...
    for ext in self._image_ext:
      ...
      mask_path = os.path.join('/data2/chenxiaoming/data/coco2014/img_train/all/mask',
                              index + ext)
