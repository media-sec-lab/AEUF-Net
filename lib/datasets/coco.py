...
from lib.config.config1 import cfg
import matplotlib
...
class coco(imdb):
  ...
  def _load_image_set_index(self):
    ...
    image_set_file = os.path.join(self._data_path,
                                  self._image_set + '.txt')  #准备好的txt
    assert os.path.exists(image_set_file), \
      'Path does not exist: {}'.format(image_set_file)
    with open(image_set_file) as f:
      image_index = [x.strip() for x in f.readlines()]  #strip()移除字符串头尾指定的字符序列,这里去掉换行符
    return image_index
...
if __name__ == '__main__':
  ...
  a = 'coco_train_filter_single'
  d = coco(a, '2007',coco_path)
  res = d.roidb
