...
def im_detect(sess, net, im):
    ...
    if cfg.FLAGS.USE_MASK is True:
        blobs['im_info'] = np.array([[im_blob.shape[1], im_blob.shape[2], im_scales[0], im.shape[0], im.shape[1]]],
                                    dtype=np.float32)
        scores1, scores, bbox_pred, rois, y_preds, mask_data = net.test_image(sess, blobs['data'],
                                                                               blobs['im_info'])
        ...
        # print(mask_scores, mask_boxes, maskcls_ind)
        # print(mask_scores.shape, mask_boxes.shape, maskcls_ind.shape)
        ...
        return scores, pred_boxes, maskcls_ind, pred_mask_boxes, mask_scores, y_preds, mask_data
    else:
...
def cal_fmeasure(precision, recall):
    ...
    return max_fmeasure
def _mask_vis(mask_pre,mask_target):
    ...
    splice=splice*255.0
    mask= tf.concat([mask_pre, splice, mask_target],2)
    return mask
def test_net(sess, net, imdb, weights_filename, max_per_image=100, thresh=0.05):
    ...
    if os.path.isfile(os.path.join(output_dir, 'detections.pkl')):
        ...
        if cfg.FLAGS.USE_MASK is True:
            ...
            # with open(os.path.join(output_dir,'my_log.txt'), 'w') as f:
            with open('./save_result/my_log.txt', 'w') as f:
                for i in range(num_images):
                    # print(output_dir)
                    print(imdb.image_path_at(i))
                    f.write('%s' % (imdb.image_path_at(i)))
                    ...
                    scores, boxes, maskcls_inds, mask_boxes, mask_scores, mask_pred, _ = im_detect(sess, net, im)
                    ...
                    if batch_ind.shape[0] == 0:
                        ...
                        m_pre = cv2.resize(mask_out, (512, 512))
                        m_gt = cv2.resize(mask_gt, (512, 512))
                        img_gt = cv2.resize(im, (512, 512))
                        _, m_pre = cv2.threshold(m_pre * 255, 127, 255, cv2.THRESH_BINARY)
                        _, m_gt = cv2.threshold(m_gt * 255, 127, 255, cv2.THRESH_BINARY)
                        cv2.imwrite('./save_result/tmp_m_pre.png'.format(i), m_pre)
                        cv2.imwrite('./save_result/tmp_m_gt.png'.format(i), m_gt)
                        tmp_m_pre = cv2.imread('./save_result/tmp_m_pre.png')
                        tmp_m_gt = cv2.imread('./save_result/tmp_m_gt.png')

                        # mask_vis = _mask_vis(mask_out,mask_gt)

                        image = np.concatenate(( img_gt, tmp_m_gt,tmp_m_pre), 1)
                        # image = np.concatenate(( m_pre, m_gt), 1)
                        # image = np.concatenate((img_gt, m_pre, m_gt), 1)
                        # cv2.imwrite('./save_result/tmp{}_f1_{}_auc_{}.png'.format(i,f1,auc_score), image)
                        # cv2.imwrite('./save_result/tmp{}_f1_{}_auc_{}_img.png'.format(i,f1,auc_score), img_gt)
                        cv2.imwrite('./save_result/tmp{}.png'.format(i), image)
                        # cv2.imwrite('./save_result/tmp{}_1.png'.format(i), img_gt)
                    ...
                    for j in range(1, imdb.num_classes):
                        f.write(' cls: %d f1:%.3f' % (j, all_f1[i, j] ))
                        f.write(' auc:%.3f' % (all_auc[i, j]))
                    f.write('\n')
                ...
                for j in range(1, imdb.num_classes):
                    ...
                    # f.write(' auc:%.3f' % (cls_auc))
                    cls_auc = cls_auc[auc_ind]
                    class_auc[j] = np.average(cls_auc)

                # f.write('all F1  Score: %.3f %.3f %.3f\n' % (class_f1[1], class_f1[2], class_f1[3]))
                # f.write('all AUC  Score: %.3f %.3f %.3f\n' %(class_auc[1],class_auc[2],class_auc[3]))
                f.write('Average F1  Score: %.3f\n' % np.average(class_f1[1:]))
                f.write('Average AUC Score: %.3f\n' % np.average(class_auc[1:]))
                # f.write('\n')
            ...
            for i in range(num_images):
                ...
                for j in range(1, imdb.num_classes):
                    ...
                    cls_dets = np.hstack((cls_boxes, cls_scores[:, np.newaxis])) \
                        .astype(np.float32, copy=False)
                    keep = nms(cls_dets, 0.2)#cfg.TEST.NMS
                    cls_dets = cls_dets[keep, :]
                    all_boxes[j][i] = cls_dets
    ...
    if cfg.FLAGS.USE_MASK is True:
        ...
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('Test Results:')
        print('Test Results:',class_f1[1:])
        # print('all F1  Score: %.3f %.3f %.3f' %(class_f1[1],class_f1[2],class_f1[3]))#only  nist
        # print('all AUC  Score: %.3f %.3f %.3f' %(class_auc[1],class_auc[2],class_auc[3]))
